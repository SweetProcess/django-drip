...
class DripsTestCase(TestCase):
    ...
    def test_day_zero_users(self):
        ...
        self.assertEqual(
            2, self.User.objects.filter(date_joined__range=(start, end)).count()
        )
    ...
    def test_day_two_users_active(self):
        ...
        self.assertEqual(
            ...
            self.User.objects.filter(
                date_joined__range=(start, end), profile__credits__gt=0
            ).count(),
        )
    ...
    def test_day_two_users_inactive(self):
        ...
        self.assertEqual(
            1,
            self.User.objects.filter(
                date_joined__range=(start, end), profile__credits=0
            ).count(),
        )
    ...
    def test_no_subject_template_drip(self):
        d = Drip.objects.create(name="A while ago", body_html_template="grate! fanx")
    ...
    def test_exclude_and_include(self):
        ...
        self.assertEqual(
            7, model_drip.drip.get_queryset().count()
    ...
    def test_admin_timeline_prunes_user_output(self):
        ...
        timeline_url = reverse(
            "admin:drip_timeline",
            kwargs={"drip_id": model_drip.id, "into_past": 3, "into_future": 3},
        )
    ...
    def test_apply_annotations_with_count(self):
        ...
        self.assertEqual(
            list(qs.query.annotation_select.keys()), ["num_profile_user_groups"]
        )
